...
import { ICoactvwc, getCoactvwcIdentifier } from '../coactvwc.model';
...
export class CoactvwcService {
  ...
  update(acctdat: ICoactvwc): Observable<EntityResponseType> {
    return this.http.put<ICoactvwc>(`${this.resourceUrl}/${getCoactvwcIdentifier(acctdat) as number}`, acctdat, { observe: 'response' });
  }

  partialUpdate(acctdat: ICoactvwc): Observable<EntityResponseType> {
    return this.http.patch<ICoactvwc>(`${this.resourceUrl}/${getCoactvwcIdentifier(acctdat) as number}`, acctdat, { observe: 'response' });
  }
